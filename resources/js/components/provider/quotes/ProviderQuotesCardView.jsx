        ...
        if (status === 'pending') {
            buttons.push(
                <button
                    ...
                    onClick={() => onQuoteAction('respond', quote)}
                >
        ...
        if (status === 'quoted') {
            ...
            buttons.push(
                <button
                    ...
                    onClick={() => onQuoteAction('withdraw', quote)}
                >
        ...
        return (
            <div className="text-center py-5">
                ...
                <p className="text-muted mb-4">
                    You haven't received any quote requests yet. Make sure your services are visible and attractive to potential clients.
                </p>
                <Link to="/provider/services" className="btn btn-primary btn-lg">
                        ...
                        <div className="card-body d-flex flex-column">
                            ...
                            <div className="d-flex justify-content-between align-items-start mb-3">
                                <div>
                                    <h6 className="card-title mb-1">
                                        ...
                                        >
                                            {quote.quote_number || `Q${String(quote.id).padStart(6, '0')}`}
                                        </Link>
                                    </h6>
                                ...
                                <div className="d-flex flex-column align-items-end">
                                    <span className={`badge ${getStatusBadge(quote.status)} mb-1`}>
                            ...
                            <div className="mb-3">
                                ...
                                {quote.service_category && (
                                    <small className="text-muted">{quote.service_category}</small>
                                )}
                            </div>
                            ...
                            <div className="mb-3">
                                <div className="d-flex align-items-center">
                                    ...
                                    <span className="me-2">{quote.client_name || quote.client?.name || 'Client'}</span>
                                    {quote.client_verified && (
                                        <i className="fas fa-check-circle text-success" title="Verified Client"></i>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="mb-3">
                                <div className="row g-2 small">
                                    <div className="col-6">
                                        ...
                                        <div className="fw-medium">{formatDate(quote.requested_date)}</div>
                                        {quote.requested_time && (
                                            <div className="text-muted small">{formatTime(quote.requested_time)}</div>
                                        )}
                                    </div>
                                    <div className="col-6">
                                        <div className="text-muted">Your Quote:</div>
                                        <div className="fw-medium text-success">
                                            {formatPrice(quote.quoted_price)}
                                        </div>
                                        {quote.travel_fee && quote.travel_fee > 0 && (
                                            <div className="small text-muted">
                                                + Rs. {quote.travel_fee} travel
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            {quote.message && (
                                <div className="mb-3">
                                    <div className="text-muted small">Client Request:</div>
                            ...
                            {quote.provider_response && (
                                <div className="mb-3">
                                    <div className="text-muted small">Your Response:</div>
                                    <p className="small mb-0 text-truncate-2">
                                        {quote.provider_response}
                                    </p>
                                </div>
                            ...
                            {quote.expires_at && quote.status === 'quoted' && (
                                <div className="mb-3">
                                    <div className="alert alert-warning alert-sm py-2 px-3 mb-0">
                                        ...
                                        <small>Expires: {formatDate(quote.expires_at)}</small>
                                    </div>
                                </div>
                            )}
                                ...
                                <div className="mb-3">
                                    <div className="d-flex align-items-center small">
                                        ...
                                        <span className="fw-bold text-success ms-1">
                                            Rs. {parseInt(quote.quoted_price + (quote.travel_fee || 0)).toLocaleString()}
                                        </span>
                                    </div>
                                </div>
                            )}
